...
final class LegacyPlaybackTests: XCTestCase {
    @MainActor
    func testOriginalPlaybackAssertions() {
        ...
        let alternate = WatchChoice(id: "alt", title: "Red Sox Multiview", kind: .liveEvent, isLive: true)
        ...
        expect(automaticChoice(from: [alternate, boxing]) == nil, "alternate Red Sox feed is not automatic")
        expect(isPrimaryRedSoxTitle("Rays at Boston Red Sox"), "primary Red Sox title recognized")
        expect(!isPrimaryRedSoxTitle("Worcester Red Sox"), "minor-league feed excluded")
        expect(!isPrimaryRedSoxTitle("Red Sox Alternate Feed"), "alternate feed excluded")
        let hdStream = WatchStream(id: "hd", title: "Rays at Red Sox")
        let uhdStream = WatchStream(id: "uhd", title: "4K: Rays at Red Sox")
        expect(preferredLiveStream([hdStream, uhdStream]) == uhdStream, "stream-level 4K rendition preferred")
